...
const AssessmentDetails = () => {
    ...
    const fetchAssessment = async () => {
      try {
        ...
        if (response.success && response.data) {
          ...
          setError(
            "Failed to load assessment details: Invalid response format"
          );
        }
      } catch (err) {
        ...
        if (
          ...
          console.warn(
            "Invalid assessment ID format, redirecting to dashboard"
          );
          navigate("/");
          return;
        }
        ...
        setError(
          `Failed to load assessment: ${err.message || "Unknown error"}`
        );
      } finally {
        setLoading(false);
      }
  ...
  const isStructuredScore = (score) => {
    return (
      typeof score === "object" &&
      score !== null &&
      (score.mark !== undefined || score.justification !== undefined)
    );
  };
  ...
  const getNumericValue = (score) => {
    ...
    if (typeof score === "object" && score !== null && score.mark) {
      ...
      const fractionWithDecimalMatch = markLower.match(
        /([\d.]+)\s*\/\s*[\d.]+/
      );
  ...
  const getStatusFromMark = (mark) => {
    ...
    if (
      markLower.includes("satisfactory") ||
    ...
    if (
      markLower.includes("needs improvement") ||
    ...
    if (
      markLower.includes("unsatisfactory") ||
  ...
  const maxPoints =
    students[0]?.assessment?.max_points || assessment.max_points || 100;
  ...
  students.forEach((student) => {
    ...
    if (student.scores && typeof student.scores === "object") {
      const scores = Object.values(student.scores).filter(
        (score) => typeof score === "number" || isStructuredScore(score)
      );
      if (scores.length > 0) {
        // Check if we have structured scores first
        const structuredScores = scores.filter((score) =>
          isStructuredScore(score)
        );
        if (structuredScores.length > 0) {
          ...
          for (const [key, value] of Object.entries(student.scores)) {
            if (
              key.toLowerCase().includes("correctness") ||
          ...
          if (mainCriterion && isStructuredScore(mainCriterion[1])) {
            ...
            if (mainStatus === "Excellent")
              scoreRanges["Excellent (90-100%)"]++;
            else if (mainStatus === "Good") scoreRanges["Good (80-89%)"]++;
            else if (mainStatus === "Satisfactory")
              scoreRanges["Satisfactory (70-79%)"]++;
            else if (mainStatus === "Needs Improvement")
              scoreRanges["Needs Improvement (60-69%)"]++;
            else if (mainStatus === "Unsatisfactory")
              scoreRanges["Unsatisfactory (0-59%)"]++;
        ...
        const avgScore =
          scores.reduce((sum, score) => sum + getNumericValue(score), 0) /
          scores.length;
              ...
              onClick={async () => {
                ...
                if (
                  ...
                  try {
                    ...
                    if (!idToDelete) {
                      alert(
                        "Assessment ID is missing or invalid. Cannot delete."
                      );
                      return;
                    }
                    ...
                    alert(
                      "Failed to delete assessment: " + (err.message || err)
                    );
                  }
                }
              }}
            ...
            <div className="w-full md:w-2/3 px-2 mb-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="flex flex-wrap">
                  {Object.entries(scoreRanges).map(
                    ([range, count]) =>
                      count > 0 && (
        ...
        <div className="p-6">
          {activeTab === "overview" && (
            <div>
              ...
              <p className="mb-4">
                This assessment contains results for {totalStudents} students.
              </p>
                    ...
                    {students.map((student, index) => {
                      ...
                      if (
                        ...
                        if (criteriaScores.length > 0) {
                          // Check if we have structured scores first
                          const structuredScores = criteriaScores.filter(
                            (score) => isStructuredScore(score)
                          );
                          if (structuredScores.length > 0) {
                            // Find the main criterion if it exists
                            let mainCriterion = null;
                            ...
                            if (
                              mainCriterion &&
                              isStructuredScore(mainCriterion[1])
                            ) {
                              // Use the main criterion's mark to determine status
                          ...
                          const avg =
                            criteriaScores.reduce(
                              (sum, item) => sum + getNumericValue(item.score),
                              0
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <div className="text-sm text-gray-900">
                              ...
                              {student.scores &&
                              Array.isArray(student.scores.extracted_scores) &&
                              student.scores.extracted_scores.length > 0 ? (
                                  ...
                                  {student.scores.extracted_scores
                                    .filter(
                                      (scoreObj) =>
                                        ...
                                        !/^\d+%$/.test(
                                          scoreObj.context.trim()
                                        ) &&
                                        !scoreObj.context.includes("mint:") &&
                                        !scoreObj.context.includes("hsl")
                                    )
                                    .slice(0, 3)
                                    .map((scoreObj, i) => (
                                      <span key={i} className="mb-1">
                                        ...
                                        {typeof scoreObj.score === "number" &&
                                          typeof scoreObj.max_score ===
                                            "number" && (
                                            <span className="ml-2 text-xs text-gray-500">
                                              ({scoreObj.score}/
                                              {scoreObj.max_score} |{" "}
                                              {Math.round(scoreObj.percentage)}
                                              %)
                                            </span>
                                          )}
                                      </span>
                                    ))}
                                  {student.scores.extracted_scores.filter(
                                    (scoreObj) =>
                                      typeof scoreObj.context === "string" &&
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-medium text-gray-900">
                              ...
                              avgScore !== "N/A"
                                ? `${avgScore}/${
                                    student.assessment?.max_points || maxPoints
                                  }`
                                : avgScore}
                            </div>
                          </td>
              ...
              {selectedStudent.error ? (
                ...
                <>
                  <div className="mb-6">
                    ...
                    typeof selectedStudent.scores === "object" ? (
                      <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                          <thead className="bg-gray-50">
                            <tr>
                          ...
                          <tbody className="bg-white divide-y divide-gray-200">
                            ...
                            {selectedStudent.scores &&
                            Array.isArray(
                              selectedStudent.scores.extracted_scores
                            ) &&
                            selectedStudent.scores.extracted_scores.length > 0
                              ? selectedStudent.scores.extracted_scores
                                  .filter(
                                    (scoreObj) =>
                                      typeof scoreObj.context === "string" &&
                            ...
                            {Object.values(selectedStudent.scores).some(
                              (score) =>
                                typeof score === "number" ||
                                isStructuredScore(score)
                            ) && (
                                ...
                                <td className="px-6 py-4 whitespace-normal text-sm font-medium text-gray-900">
                                  {(() => {
                                    ...
                                    const avg =
                                      scores.reduce(
                                        (sum, score) =>
                                          sum + getNumericValue(score),
                                        0
                                      ) / scores.length;
                                    return `${avg.toFixed(1)}/${
                                      selectedStudent.assessment?.max_points ||
                                      maxPoints
                                    }`;
                                  })()}
                                </td>
